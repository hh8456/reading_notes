         ...
         ;计算GDT所在的逻辑段地址
		 ; // 将 GDT 线性基地址存放在 EAX; 标号 pgdt 的类型是 dw - 字 - 两个字节,所以 + 0x02 就取得了 GDT的物理地址 dd 0x00007e00 
         mov eax,[cs:pgdt+0x7c00+0x02]      ;GDT的32位线性基地址 
         xor edx,edx
		 ; // 64位被除数在 edx:eax 中, 32位除数在 ebx 中
         mov ebx,16
		 ; // 指令执行后, eax 中的商是段地址, 仅低 16 位有效, edx 中的余数是段内偏移地址
         div ebx                            ;分解成16位逻辑地址 
